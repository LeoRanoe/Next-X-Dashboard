                ...
                <div key={order.id} className="p-4 lg:p-5 hover:bg-muted/50 transition-colors">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {/* Action buttons */}
                      <Button onClick={() => handleViewOrder(order)} variant="ghost" size="sm">
                        <Eye size={14} />
                      </Button>
                      {order.status === 'pending' && (
                        <Button onClick={() => handleEditOrder(order)} variant="ghost" size="sm">
                          <Edit size={14} />
                        </Button>
                      )}
                      {(order.status === 'pending' || order.status === 'cancelled') && (
                        <Button onClick={() => handleDeleteOrder(order)} variant="ghost" size="sm" className="text-destructive hover:text-destructive">
                          <Trash2 size={14} />
                        </Button>
                      )}
                    </div>
        ...
        <form onSubmit={handleSubmitOrder} className="space-y-4">
          ...
          <div className="border border-border rounded-lg p-4">
            <div className="flex justify-between items-center mb-3">
              ...
              <Button type="button" onClick={addOrderItem} variant="secondary" size="sm">
                <Plus size={14} />
                Add Item
              </Button>
            </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button type="button" onClick={resetOrderForm} variant="secondary">
              Cancel
            </Button>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{viewingOrder.wallets?.person_name} - {viewingOrder.wallets?.type}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">To Location</span>
                <p className="font-medium">{viewingOrder.locations?.name}</p>
              </div>
              <div>
              ...
              {viewingOrder.expected_arrival && (
                <div>
                  ...
                  <p className="font-medium">{new Date(viewingOrder.expected_arrival).toLocaleDateString()}</p>
                </div>
              )}
            </div>
              ...
              <div className="divide-y divide-border">
                {viewingOrder.purchase_order_items?.map((item) => (
                  <div key={item.id} className="px-4 py-3 flex justify-between items-center">
                    ...
                    <span className="font-medium">{formatCurrency(item.subtotal, viewingOrder.currency as Currency)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <Button onClick={() => { setShowViewOrder(false); setViewingOrder(null) }} variant="secondary">
                Close
              </Button>
            </div>
          </div>
